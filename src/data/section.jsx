export const section = [
    {
        id: "about",
        title: "About Me",
        image: "/artworks/van-gogh.jpg",
        type: "single",
        content: {
            photo: "/.png",
            description: "Halo! Nama saya Mutiara Sayla, seorang lulusan Teknik Informatika dari Politeknik Negeri Batam, Indonesia. Saya mengkhususkan diri dalam bidang programming dan memiliki minat tinggi dalam di dunia IT. Dengan pengalaman magang di beberapa perusahaan IT, saya unggul dalam menyelesaikan beberapa proyek pengembangan software. Diantaranya saya pernah mengembangkan Automation Center Website,Smart Factory 3D, Modul Training System dalam bentuk Augmented Reality. Dan saat ini saya terbuka untuk mendapatkan pengalaman lainnya di bidang ini"
        }
    },
    {
        id: "resume",
        title: "Resume",
        image: "/artworks/van-gogh.jpg",
        type: "file",
        content: {
            file: "/export.pdf"
        }
    },
    {
        id: "education",
        title: "Education",
        image: "/artworks/van-gogh.jpg",
        type: "timeline",
        content: [
            {
                year: "2021 - 2024",
                title: "Politeknik Negeri Batam",
                description: "D3 Teknik Inforamtika"
            },
            {
                year: "2018 - 2021",
                title: "SMK Hasanah Pekanbaru",
                description: "Jurusan Rekayasa Perangkat Lunak"
            },
            {
                year: "2016 - 2018",
                title: "MTSN 3 Padang Pariaman",
                description: ""
            },
            {
                year: "2010 - 2016",
                title: "SDN 43 Pekanbaru",
                description: ""
            }
        ]
    },
    {
        id: "experience",
        title: "Experience",
        image: "/artworks/van-gogh.jpg",
        type: "timeline",
        content: [
            {
                year: "2025 - 2026",
                title: "PT. Pegaunihan Technology Indonesia",
                description: "Automation Engineer"
            },
            {
                year: "2024 - 205",
                title: "Elsoft Indonesia",
                description: "Software Implementer"
            },
            {
                year: "2023 - 2024",
                title: "PT. Labtech Penta International",
                description: "Unity Programmer Internship"
            },
            {
                year: "2022 -2023",
                title: "Infinite Learning",
                description: "MSIB Hybrid Cloud & AI"
            },
            {
                year: "2017 -2018",
                title: "PT. Garuda Cyber Indonesia",
                description: "Administration & Web Development Internship"
            }
        ]
    },
    {
        id: "community",
        title: "Community",
        image: "/artworks/van-gogh.jpg",
        type: "timeline",
        content: [
            {
                year: "2023- 2024",
                title: "Dewan Perwakilan Mahasiswa",
                description: "Sekretaris I"
            },
            {
                year: "2022 - 2023",
                title: "Generasi Baru Bank Indonesia",
                description: "Anggota Kesehatan Masyarakat"
            },
            {
                year: "2021 - 2022",
                title: "Batam Linux User Group",
                description: "Sekretaris II"
            }
        ]
    },
    {
        id: "projects",
        title: "Projects",
        image: "/artworks/van-gogh.jpg",
        type: "gallery",
        content: [
            {
                title: "Automation Center Website - Vue TS",
                description: "",
            },
            {
                title: "3D Shelf Smart Factory Website - Three JS",
                description: ""
            },
            {
                title: "Timesheet & Payroll system for Logistics Company - ERP Implementation ",
                description: ""
            },
            {
                title: "Augmented Reality  Evaporator Air Cooled, Forced Air - Unity AR",
                description: ""
            },
            {
                title: "SMK Website - Wordpress Web",
                description: ""
            },

        ]
    },
    {
        id: "certificates",
        title: "Certificates",
        image: "/artworks/van-gogh.jpg",
        type: "gallery",
        content: [
            {
                title: "Red Hat Certified System Administrator (RHCSA)",
                image: "/rhsca.png"
            },
            {
                title: "Python for Data Science",
                image: "/phyton.png"
            }
        ]
    },
    {
        id: "socialMedia",
        title: "Social Media",
        image: "/artworks/van-gogh.jpg",
        type: "icon",
        content: [
            {
                title: "LinkedIn",
                link: "https://linkedin.com/in/mutiara-sayla"
            },
            {
                title: "Github",
                link: "https://github.com/sayla-stack"
            }
        ]
    }
]

